...
def test_serializer_standard(admin_user):
    client = APIClient()
    response = client.get('/users/?serializer=std', format='json')
    assert response.status_code == 200
...
def test_serializer_light(admin_user):
    client = APIClient()
    response = client.get('/users/?serializer=light', format='json')
...
def test_serializer_short(admin_user):
    client = APIClient()
    response = client.get('/users/?serializer=short', format='json')
...
def test_serializer_broken(admin_user):
    ...
    with pytest.raises(InvalidSerializerError):
        client.get('/users/?serializer=broken', format='json')


@pytest.mark.django_db()
def test_serializer_wrong(admin_user):
    client = APIClient()
    response = client.get('/users/?serializer=wrong', format='json')
    assert set(response.json()[0].keys()) == {'email', 'first_name',
                                              'groups', 'is_active',
                                              'is_staff', 'last_login',
                                              'user_permissions', 'url',
...
def test_serializer_none(admin_user):
    client = APIClient()
    response = client.get('/users/?serializer=none', format='json')
    assert set(response.json()[0].keys()) == {'email', 'first_name',
                                              'groups', 'is_active',
                                              'is_staff', 'last_login',
                                              'user_permissions', 'url',
